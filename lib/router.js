...
export default class Router {
  constructor(app, options={}) {
    ...
    this.routerWindow = this.window = options.window || window;
  ...
  registerListeners(historyMethod) {
    this.navigateToHash = () => this.navigate(this.routerWindow.location.hash);
    this.routerWindow.addEventListener(`popstate`, this.navigateToHash);

    this.historyMethod = historyMethod;
    this.origChangeStateMethod = this.routerWindow.history[this.historyMethod];
    this.routerWindow.history[this.historyMethod] = function() {
      this.origChangeStateMethod.apply(this.routerWindow.history, arguments);
      this.navigateToHash();
    };
  }

  unregisterListeners() {
    this.routerWindow.removeEventListener(`popstate`, this.navigateToHash);
    this.routerWindow.history[this.historyMethod] = this.origChangeStateMethod;
  }
