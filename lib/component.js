...
class Component extends WebComponent {
   ...
   * @property {array} [sharedStateKeys] - keys of top-level state entries to share with parent app;
   * if not set, child component shares entire state object
  ...
  /**
   * Returns only those key-value pairs in an object relevant to shared
   * parent-child app state. If this component has specified sharedStateKeys
   * in its configuration, only those entries are returned; otherwise the
   * original parameter object is returned.
   * @param {object} [obj] - object to pick (usually a state object or state update)
   * @returns object with key-value entries picked from parameter obj
   * @example
   * myWidget.sharedStateEntries({foo: 'bar', baz: 'quux'});
   * // if myWidget.getConfig('sharedStateKeys') is ['foo'], returns:
   * // {foo: 'bar'}
   */
  sharedStateEntries(obj) {
    const sharedStateKeys = this.getConfig(`sharedStateKeys`);
    return sharedStateKeys ? pick(obj, sharedStateKeys) : obj;
  }
  ...
  update(stateUpdate={}) {
    if (!this.initialized) {
      ...
      this._rootUpdate(stateUpdate);
    } else {
      this._rootUpdate(stateUpdate);
      this.$panelRoot._rootUpdate(this.sharedStateEntries(stateUpdate), {exclude: this});
    }
  }
  ...
  attachedCallback() {
    ...
    if (typeof this.$panelParentID !== 'undefined') {
      ...
      this.state = this.sharedStateEntries(this.$panelRoot.state);
    } else {
  ...
  updateSelfAndChildren(stateUpdate, options={}) {
    if (this.initialized && this.shouldUpdate(stateUpdate)) {
      ...
      this.domPatcher.update(newState);
      for (let child of this.$panelChildren) {
        if (options.exclude !== child) {
          child.updateSelfAndChildren(child.sharedStateEntries(newState), options);
        }
      }
    }
  }
