  ...
  disconnectedCallback() {
    ...
    Promise.resolve().then(() => {
      ...
      if (!this.initialized) {
        ...
        [`appState`, `state`, `app`, `updateApp`, `update`].forEach(function replaceWithErrorAccessor(prop) {
          Object.defineProperty(this, prop, {
            get() {
              throw new AccessAfterDisconnectError(this.tagName, prop);
            },
          });
        });
      }
...
class AccessAfterDisconnectError extends Error {
  constructor(tagName, method) {
    super(`"${tagName}"#${method} called after disconnectedCallback`);

    this.name = `AccessAfterDisconnectError`
