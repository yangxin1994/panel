  ...
  _updateAttr(attr) {
    ...
    if (attrsSchema.hasOwnProperty(attr)) {
      ...
      if (!this.hasAttribute(attr) && !this._attrs.hasOwnProperty(attr)) {
        if (attrSchema.default === true && attrType === `boolean`) {
          // Defaulting a boolean to true is problematic because any subsequent call
          // to `removeAttribute` will not actually trigger an attributeChangedCallback.
          // The browser sees the value as null and so there is no need to remove it, this
          // hack will set the value explicitly to a boolean attribute of true so that it
          // behaves correctly. (see: https://codesandbox.io/s/panel-default-boolean-true-v03ku)
          requestAnimationFrame(() => this.setAttribute(attr, ``));
        }
