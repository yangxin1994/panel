    ...
    if (attrsSchema.hasOwnProperty(attr)) {
      ...
      if (!this.hasAttribute(attr)) {
        if (attrType === `boolean` && attrSchema.default) {
          console.warn(`${this}: attr '${attr}' is defaulted to true. boolean attributes default true is deprecated and will be removed with the next release`);
        }
        if (this._attrs.hasOwnProperty(attr)) {
          console.warn(`${this}: overriding already set attribute '${attr}' (${this._attrs[attr]}) with default value ${attrSchema.default}`)
        }
