...
class Component extends WebComponent {
  ...
  _updateAttr(attr) {
    ...
    if (attrsSchema.hasOwnProperty(attr)) {
      ...
      if (!this.hasAttribute(attr) && !this._attrs.hasOwnProperty(attr)) {
  ...
  removeAttribute(attr) {
    // Defaulting a boolean to true is problematic because any calls to `removeAttribute`
    // will not actually trigger an attributeChangedCallback. The browser sees the value
    // as null and so there is no need to remove it, this hack will ensure the components
    // internal state is properly updated to false when removing a default true boolean.
    // see: https://codesandbox.io/s/panel-default-boolean-true-v03ku
    const attrsSchema = this._attrsSchema;
    if (!this.hasAttribute(attr) && attrsSchema.hasOwnProperty(attr)) {
      const attrSchema = attrsSchema[attr];
      if (attrSchema.type === `boolean` && (attrSchema.default === true || this._attrs[attr] === true)) {
        this.attributeChangedCallback(attr, ``, null);
      }
    }
